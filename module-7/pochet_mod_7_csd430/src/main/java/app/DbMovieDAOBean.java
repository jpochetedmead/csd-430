 ...
 * Purpose: JDBC helper bean to create/seed/drop table, INSERT rows, and READ records.
 */
package app;
...
public class DbMovieDAOBean {
    ...
    public List<Integer> getAllIds() throws Exception {
        ...
        try (Connection c = DriverManager.getConnection(url, user, pass);
             PreparedStatement ps = c.prepareStatement(
                 "SELECT movie_id FROM julio_movies_data ORDER BY movie_id");
    ...
    // ---- Module 7 additions ----
    // CREATE: insert a new movie; returns the generated primary key
    public int insertMovie(MovieBean m) throws Exception {
        ...
        try (Connection c = DriverManager.getConnection(url, user, pass);
            ...
            if (affected == 0) throw new SQLException("Insert failed: 0 rows affected.");
    ...
    // READ: return ALL rows (for listing after a create)
